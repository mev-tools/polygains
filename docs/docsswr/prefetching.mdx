...
function App({ userId }) {
  ...
  // preload in effects
  useEffect(() => {
    preload('/user?id=' + userId, fetcher)
  }, [userId])
  ...
  return (
    <div>
      <button
        ...
        onHover={() => preload('/user?id=' + userId, fetcher)}
      >
        Show User
      </button>
      {show ? <User /> : null}
...
const Page = () => {
  // The below useSWR hooks will suspend the rendering, but the requests to `/user` and `/movies` have started by `preload` already,
  // so the waterfall problem doesn't happen.
  const { data: user } = useSWR('/user', fetcher, { suspense: true });
  const { data: movies } = useSWR('/movies', fetcher, { suspense: true });
  return (
    <div>
      <User user={user} />
...
```jsx
useSWR('/data', fetcher, { fallbackData: prefetchedData })
```
