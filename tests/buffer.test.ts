...
import { WindowBuffer, InsiderEvaluator, TraderData } from "@/services/buffer";
...
describe("WindowBuffer", () => {
    ...
    test("should return empty if currentTimestamp is undefined or NaN", () => {
        ...
        expect(Object.keys(buffer.flush(undefined as any)).length).toBe(0);
    ...
    test("should detect insider if volume threshold met", () => {
        const flushedData: Record<string, TraderData> = {
            "t1": {
                id: "t1",
                tokenstats: {},
                userStats: { tradeVol: VOLUME_THRESHOLD + 1n, tradeCount: 1, firstSeen: 1000 }
            }
        };
    ...
    test("should skip if already known as non-insider", () => {
        ...
        const flushedData: Record<string, TraderData> = {
            "t1": {
                id: "t1",
                tokenstats: {},
                userStats: { tradeVol: VOLUME_THRESHOLD + 100n, tradeCount: 1, firstSeen: 1000 }
            }
        };
