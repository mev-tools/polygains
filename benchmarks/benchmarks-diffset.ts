...
const HASHES = 4;
const _XXHASH_PRIME1 = 11400714785074694791n;
const _XXHASH_PRIME2 = 14029467366897019727n;
const _XXHASH_PRIME3 = 1609587929392839161n;
const _XXHASH_PRIME4 = 965002924228719859n;
...
const currentFilter = new XXHashBloomFilter();
let currentSize = 0;
...
console.log(
	`   Misses: ${currentMisses}/${currentLookups} (${((currentMisses / currentLookups) * 100).toFixed(1)}%)`,
);
console.log(`   Note: Misses include false positives + actual misses\n`);

console.log("2. CUSTOM: xxHash32-based BloomFilter");

const xxFilter = new XXHashBloomFilter();
let _xxSize = 0;
let xxLookups = 0;
let xxMisses = 0;
...
console.log(
	`   Misses: ${xxMisses}/${xxLookups} (${((xxMisses / xxLookups) * 100).toFixed(1)}%)\n`,
);
...
const diffSet = new DiffSetDetector();
let _diffSize = 0;
let diffLookups = 0;
let diffMisses = 0;
...
console.log(
	`   Memory: ~${(diffSet.getMemoryBytes() / 1024 / 1024).toFixed(1)}MB`,
);
console.log(`   Total: ${diffStats.total}, Diff: ${diffStats.diff}`);
console.log(
	`   Misses: ${diffMisses}/${diffLookups} (${((diffMisses / diffLookups) * 100).toFixed(1)}%)`,
);
...
const bfSnapshotTime = measure(() => {
	const buckets = (currentFilter as unknown as FilterInternals).buckets;
...
const savings = (
	((bfSnapshotTime - diffSnapshotTime) / bfSnapshotTime) *
	100
).toFixed(1);
...
const workloadSpeedup = (
	((bfWorkloadTime - diffWorkloadTime) / bfWorkloadTime) *
	100
).toFixed(1);
...
const _dsLookup = (diffLookups / ((now() - now() + 1) * 1000)).toFixed(0);
console.log(
	`Lookups/sec              | ${bfLookup}        | ~2M         | ~2M`,
);
