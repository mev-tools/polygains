...
const TOTAL_ADDRESSES = BATCHES * ADDRESSES_PER_BATCH;
...
measure(() => {
    for (let b = 0; b < BATCHES; b++) {
        ...
        for (let i = 0; i < ADDRESSES_PER_BATCH; i++) {
            batch.push(`0x${(b * ADDRESSES_PER_BATCH + i).toString(16).padStart(40, "0")}`);
        }
...
class ForEachDetector {
    ...
    addMany(addresses: Iterable<string>): void {
        ...
        hashes.forEach(h => this.set.add(h));
    }
...
console.log("----------------|----------|----------|----------");
console.log(`Loop + add       | ${avg(currentTimes).toFixed(2)}   | ${min(currentTimes).toFixed(2)}   | ${max(currentTimes).toFixed(2)}`);
console.log(`Set.union        | ${avg(unionTimes).toFixed(2)}   | ${min(unionTimes).toFixed(2)}   | ${max(unionTimes).toFixed(2)}`);
console.log(`Spread + Set      | ${avg(spreadTimes).toFixed(2)}   | ${min(spreadTimes).toFixed(2)}   | ${max(spreadTimes).toFixed(2)}`);
console.log(`forEach + add     | ${avg(forEachTimes).toFixed(2)}   | ${min(forEachTimes).toFixed(2)}   | ${max(forEachTimes).toFixed(2)}`);
...
const spreadSpeedup = ((currentAvg - spreadAvg) / currentAvg * 100).toFixed(1);
const forEachSpeedup = ((currentAvg - forEachAvg) / currentAvg * 100).toFixed(1);
...
if (parseFloat(spreadSpeedup) > 0) {
    ...
    console.log(`✗ Spread + Set is ${Math.abs(parseFloat(spreadSpeedup))}% slower`);
}

if (parseFloat(forEachSpeedup) > 0) {
    console.log(`✓ forEach + add is ${forEachSpeedup}% faster than loop + add`);
} else {
    console.log(`✗ forEach + add is ${Math.abs(parseFloat(forEachSpeedup))}% slower`);
}
...
console.log(`   ${((currentAvg - methods[0].time) / currentAvg * 100).toFixed(1)}% faster than current`);

console.log("\nRanking:");
methods.forEach((m, i) => {
    const diff = ((currentAvg - m.time) / currentAvg * 100).toFixed(1);
    console.log(`  ${i + 1}. ${m.name.padEnd(15)} - ${diff > 0 ? '+' : ''}${diff}%`);
});
...
if (methods[0].name === "Set.union") {
    ...
    console.log("          : Array.from(addresses).map(a => Bun.hash.xxHash32(a));");
