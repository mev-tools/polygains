...
test.describe('Frontend E2E Tests', () => {
  ...
  test('should have working API health endpoint', async ({ page, request }) => {
  ...
  test('should load global stats from API', async ({ page, request }) => {
    const apiBaseUrl = process.env.API_BASE_URL || 'http://localhost:4000';
  ...
  test('should load markets from API', async ({ request }) => {
    const apiBaseUrl = process.env.API_BASE_URL || 'http://localhost:4000';
    const response = await request.get(`${apiBaseUrl}/api/markets?page=1&limit=10`);
  ...
  test('should handle 404 for non-existent routes', async ({ request }) => {
    const apiBaseUrl = process.env.API_BASE_URL || 'http://localhost:4000';
    const response = await request.get(`${apiBaseUrl}/this-route-does-not-exist`);
