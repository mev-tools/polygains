...
console.log("------------------|----------------|-----------------|-----------");
console.log(`BloomFilter       | ~100KB         | ${bfLookupTime.toFixed(1)}ms          | ${bfRealTime.toFixed(1)}ms`);
console.log(`XXHash32Set V2   | ~1.6MB         | ${xxLookupTime.toFixed(1)}ms          | ${xxRealTime.toFixed(1)}ms`);
console.log(`PureSet          | ~10MB           | ${psLookupTime.toFixed(1)}ms          | ${psRealTime.toFixed(1)}ms`);
...
if (parseFloat(bfVsXx) > 0) {
    ...
    console.log(`✗ XXHash32Set V2 is ${Math.abs(parseFloat(bfVsXx))}% SLOWER than BloomFilter`);
}

if (parseFloat(bfVsPs) > 0) {
    console.log(`✓ PureSet is ${bfVsPs}% FASTER than BloomFilter`);
} else {
    console.log(`✗ PureSet is ${Math.abs(parseFloat(bfVsPs))}% SLOWER than BloomFilter`);
}
