...
```jsx
try {
  const user = await mutate('/user', updateUser(newUser))
} catch (error) {
...
function Profile() {
  ...
  const { trigger } = useSWRMutation('/user', updateUser, options)
...
function App() {
  const { trigger, isMutating } = useSWRMutation('/user', sendRequest, /* options */)
...
```jsx
const { trigger, data, error } = useSWRMutation('/user', sendRequest)
```
...
```jsx
const { trigger } = useSWRMutation('/user', updateUser, {
...
const Page = () => {
  ...
  const { data: user, trigger } = useSWRMutation('/user', fetcher);
  ...
  return (
    <div>
      ...
      <button onClick={async () => {
        ...
        mutate('/user', updateFn(user), options);
      }}>Uppercase my name!</button>
    </div>
  )
}
...
function Profile () {
  const { trigger } = useSWRMutation('/user', updateUserName)
...
```jsx
const updateTodo = () => fetch('/todos/1', {
  method: 'PATCH',
  body: JSON.stringify({ completed: true })
})

mutate('/todos', updateTodo, {
...
```tsx
function Profile() {
  const { data } = useSWR('/user', getUser, { revalidateInterval: 3000 })
  const { trigger } = useSWRMutation('/user', updateUser)
...
```jsx
mutate('/todos', async todos => {
  // let's update the todo with ID `1` to be completed,
  // this API returns the updated data
  const updatedTodo = await fetch('/todos/1', {
    method: 'PATCH',
    body: JSON.stringify({ completed: true })
  })
...
mutate(
  key => typeof key === 'string' && key.startsWith('/item?id='),
  undefined,
  { revalidate: true }
)
```
...
mutate((key: any) => /\/.test(key.toString()))
```
