...
const minHeap = new Heap((a: TraderData, b: TraderData) => a.userStats.firstSeen - b.userStats.firstSeen);
...
measure(() => {
    while (
        minHeap.length > 0 &&
        currentTimestamp - minHeap.peek()!.userStats.firstSeen >= W_SIZE_MS
    ) {
