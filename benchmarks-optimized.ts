...
measure(() => {
    ...
    for (let i = 0; i < NUM_LOOKUPS; i++) {
        ...
        if (i < NUM_LOOKUPS * 0.7) {
            ...
            lookupAddresses.push(`0x${(NUM_ELEMENTS + i).toString(16).padStart(40, "0")}`);
        }
    }
...
let bf = new BloomFilter(BITS, HASHES);
...
let pureSet = new PureSetDetector();
...
let nativeSet = new NativeSetDetector();
...
for (let i = 0; i < NUM_LOOKUPS; i++) {
    ...
    if (i < NUM_LOOKUPS / 2) {
        ...
        newOrderAddresses.push(`0x${(NUM_ELEMENTS + i).toString(16).padStart(40, "0")}`);  // new
    }
}
...
console.log("Operation           | BloomFilter | XXHash32Set | PureSet  | Native (num)");
console.log("---------------------|-------------|-------------|----------|--------------");

const addSpeedup = ((bfAddTime - xxAddTime) / bfAddTime * 100).toFixed(1);
const lookupSpeedup = ((bfLookupTime - xxLookupTime) / bfLookupTime * 100).toFixed(1);
const realSpeedup = ((bfRealAdd - xxRealAdd) / bfRealAdd * 100).toFixed(1);

console.log(`Add ${NUM_ELEMENTS}      | ${bfAddTime.toFixed(1)}ms    | ${xxAddTime.toFixed(1)}ms     | ${psAddTime.toFixed(1)}ms | ${nsAddTime.toFixed(1)}ms`);
console.log(`Test ${NUM_LOOKUPS}    | ${bfLookupTime.toFixed(1)}ms   | ${xxLookupTime.toFixed(1)}ms    | ${psLookupTime.toFixed(1)}ms | ${nsLookupTime.toFixed(1)}ms`);
console.log(`Realistic workload   | ${bfRealAdd.toFixed(1)}ms    | ${xxRealAdd.toFixed(1)}ms     | ${psRealAdd.toFixed(1)}ms | ~250ms (est)`);
...
if (parseFloat(addSpeedup) > 0) {
    ...
    console.log(`✗ XXHash32Set ADD is ${Math.abs(parseFloat(addSpeedup))}% slower than BloomFilter`);
}

if (parseFloat(lookupSpeedup) > 0) {
    console.log(`✓ XXHash32Set TEST is ${lookupSpeedup}% faster than BloomFilter`);
} else {
    console.log(`✗ XXHash32Set TEST is ${Math.abs(parseFloat(lookupSpeedup))}% slower than BloomFilter`);
}

if (parseFloat(realSpeedup) > 0) {
    console.log(`✓ XXHash32Set REALISTIC is ${realSpeedup}% faster than BloomFilter`);
} else {
    console.log(`✗ XXHash32Set REALISTIC is ${Math.abs(parseFloat(realSpeedup))}% slower than BloomFilter`);
}
